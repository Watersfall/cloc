...
public class NationArmy extends NationBase
{
	...
	{
		return this.size * this.size * this.training / 200;
	}

	public double getPower()
	{
		double power = size * 1000;
		if(power > musket)
		{
			power = power * (musket / power);
		}
	...
	public int getAttackingCasualties(NationArmy defender)
	{
		...
		if(attackPower > defensePower)
		{
			...
			double armyHp = this.size * 20.0;
			armyHp -= defensePower;
			return (int)(this.size - (armyHp / 20.0));
		}
		else
		{
	...
	public int getDefendingCasualties(NationArmy attacker)
	{
